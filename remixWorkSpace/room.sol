...
contract room is Ownable { 
   ...
   // 验证谁输谁赢
   function judg(address verifierAddr,uint[3][2] memory proof, uint[3] memory input) public returns(bool) {
   ...
   function giveUp() external {
      ...
      address opponent = pairs[msg.sender]; 
      liquidation(opponent, msg.sender);
   }
   // 赌博过程结束，结算函数
   function liquidation(address winner,address loser) internal {
